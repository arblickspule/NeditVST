// The Generate page (docs/ui-layout-decision.md migration step 2): every
// Beats-mode section composed on the layout DSL as a vertical stack of
// TitledPanels. Reads model state through the shared 10fps poll
// (syncFromModel()) and writes back through each control's own callback --
// the same data flow as the old PluginEditor, with none of its setBounds()
// math. Model/engine are only ever touched through their public members.
...
//==============================================================================
GeneratePage::GeneratePage (SlicerModel& modelToUse, SlicerEngine& engineToUse, WaveformDisplay& waveformToUse)
    : Box (true),
      model (modelToUse),
      engine (engineToUse),
      waveform (waveformToUse)
{
    setGap (14); // section-to-section gap
    setPadding (juce::BorderSize<int> (0, 0, 8, 0));

    lastGenerateTriggerMode = model.getTriggerMode();
...
//==============================================================================
void GeneratePage::buildSections()
{
...
TitledPanel* GeneratePage::addSection (std::vector<Cell>& cells, juce::String title)
{
...
//==============================================================================
void GeneratePage::buildSampleSection()
{
...
//==============================================================================
void GeneratePage::buildTempoSection()
{
    ...
    auto bpmRow = std::make_unique<ui::Row> (std::vector<Cell>{});
...
void GeneratePage::rebuildManualBpmRow()
{
...
//==============================================================================
void GeneratePage::buildDetectionSection()
{
    ...
    auto gridRow = std::make_unique<ui::Row> (std::vector<Cell>{});
...
void GeneratePage::rebuildQuantizeGridRow()
{
...
//==============================================================================
void GeneratePage::buildFadeSection()
{
...
//==============================================================================
void GeneratePage::buildPitchModeSection()
{
    ...
    auto extra = std::make_unique<ui::Column> (std::vector<Cell>{});
...
void GeneratePage::rebuildPitchModeExtra()
{
    std::vector<Cell> cells;
...
//==============================================================================
void GeneratePage::buildPlaybackStyleSection()
{
    playbackStyleSection->setCells (std::vector<Cell>{});
    rebuildPlaybackStyleContent();
}

void GeneratePage::rebuildPlaybackStyleContent()
{
    auto segments = std::make_unique<SegmentedButtonRow>();
    segments->setSize (400, SegmentedButtonRow::preferredHeight);
    {
...
//==============================================================================
void GeneratePage::buildTimingSection()
{
...
void GeneratePage::rebuildTimingExtra()
{
...
std::vector<Cell> GeneratePage::makeTimingClockCells()
{
    std::vector<Cell> cells;
...
std::vector<Cell> GeneratePage::makeTimingSliceLengthCells()
{
    std::vector<Cell> cells;
...
//==============================================================================
void GeneratePage::syncFromModel()
{
...
void GeneratePage::refreshAfterSampleOrSliceChange()
{
...
//==============================================================================
std::unique_ptr<juce::Label> GeneratePage::makeLabel (const juce::String& text)
{
...
std::unique_ptr<juce::Slider> GeneratePage::makeSlider (juce::Slider::SliderStyle style,
...
std::unique_ptr<juce::ComboBox> GeneratePage::makeComboBox (int width, int height)
{
...
std::unique_ptr<juce::ToggleButton> GeneratePage::makeToggle (const juce::String& text)
{
